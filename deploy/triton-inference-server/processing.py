...
def preprocess(img, input_shape, letter_box=True):
    if letter_box:
        ...
        img = cv2.resize(
            img, (input_shape[1], input_shape[0]), interpolation=cv2.INTER_CUBIC)
...
def postprocess(num_dets, det_boxes, det_scores, det_classes, img_w, img_h, input_shape, letter_box=True):
    ...
    if letter_box:
        boxes -= np.array([offset_w, offset_h, offset_w,
                          offset_h], dtype=np.float32)
    boxes = boxes.astype(np.int)

    detected_objects = []
    for box, score, label in zip(boxes, scores, classes):
        detected_objects.append(BoundingBox(
            label, score, box[0], box[2], box[1], box[3], img_w, img_h))
