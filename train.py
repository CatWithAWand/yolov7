...
def train(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        run_id = torch.load(
            weights, map_location=device).get('wandb_id') if weights.endswith(
    ...
    train_path = data_dict['train'] if os.path.exists(
        data_dict['train']) else dataset_dir + data_dict['train'].replace(
            '..', '')
    test_path = data_dict['val'] if os.path.exists(
        data_dict['val']) else dataset_dir + data_dict['val'].replace('..', '')
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            if Path('evolve.txt').exists(
                ...
                while all(
                        v ==
                        1):  # mutate until a change occurs (prevent duplicates)
