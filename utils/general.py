...
def check_requirements(requirements='requirements.txt', exclude=()):
    ...
    for r in requirements:
        try:
            ...
            n += 1
            print(
                f"{prefix} {e.req} not found and is required by YOLOv7, attempting auto-update...")
...
def check_dataset(dict, dataset_dir):
    ...
    val_path = dict['val'] if os.path.exists(
        dict['val']) else dataset_dir + dict['val'].replace('..', '')
        ...
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                ...
                im = cv2.resize(cutout, (224, 224),
                                interpolation=cv2.INTER_CUBIC)  # BGR
