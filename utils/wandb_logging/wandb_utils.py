    ...
    def __init__(self, opt, name, run_id, data_dict, job_type='Training'):
        ...
        if isinstance(opt.resume, str):  # checks resume from artifact
            ...
            self.wandb_run = wandb.init(config=opt,
                                        resume="allow",
                                        project='YOLOv7' if opt.project == 'runs/train' else Path(
                                            opt.project).stem,
                                        name=name,
                                        job_type=job_type,
        ...
        if self.wandb_run:
            ...
            print(
                f"{prefix}Install Weights & Biases for YOLOv7 logging with 'pip install wandb' (recommended)")

    def check_and_upload_dataset(self, opt):
        assert wandb, 'Install wandb to upload dataset'
        check_dataset(self.data_dict, os.path.dirname(opt.data))
